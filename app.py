...
@app.route('/tracking/camera/tmc/registers')
def tmc_registers():
    try:
        if stepper_controller is None:
            return jsonify({'status': 'error', 'message': 'Stepper controller not available'}), 503
        regs = stepper_controller.get_tmc_registers()
        if regs is None:
            return jsonify({'status': 'error', 'message': 'UART not enabled or driver unavailable'}), 400
        return jsonify({'status': 'success', 'registers': regs})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 400

@app.route('/tracking/camera/tmc/apply_defaults', methods=['POST'])
def tmc_apply_defaults():
    try:
        if stepper_controller is None:
            return jsonify({'status': 'error', 'message': 'Stepper controller not available'}), 503
        data = request.get_json() or {}
        microsteps = data.get('microsteps')
        ok = stepper_controller.tmc_apply_defaults(microsteps)
        if not ok:
            return jsonify({'status': 'error', 'message': 'Failed to apply defaults. Ensure UART is enabled and reachable.'}), 400
        return jsonify({'status': 'success', 'message': 'Defaults applied', 'microsteps': microsteps})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 400

@app.route('/tracking/camera/tmc/ihold_irun', methods=['POST'])
def tmc_set_ihold_irun():
    try:
        if stepper_controller is None:
            return jsonify({'status': 'error', 'message': 'Stepper controller not available'}), 503
        data = request.get_json() or {}
        axis = str(data.get('axis', 'x')).lower()
        ihold = int(data.get('ihold'))
        irun = int(data.get('irun'))
        iholddelay = int(data.get('iholddelay'))
        if axis not in ['x','y','both']:
            return jsonify({'status': 'error', 'message': 'axis must be x, y, or both'}), 400
        ok = True
        if axis in ['x','both']:
            ok = ok and stepper_controller.tmc_set_ihold_irun('x', ihold, irun, iholddelay)
        if axis in ['y','both']:
            ok = ok and stepper_controller.tmc_set_ihold_irun('y', ihold, irun, iholddelay)
        if not ok:
            return jsonify({'status': 'error', 'message': 'Failed to write IHOLD_IRUN'}), 400
        return jsonify({'status': 'success'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 400

@app.route('/tracking/camera/tmc/tpowerdown', methods=['POST'])
def tmc_set_tpowerdown():
    try:
        if stepper_controller is None:
            return jsonify({'status': 'error', 'message': 'Stepper controller not available'}), 503
        data = request.get_json() or {}
        axis = str(data.get('axis', 'x')).lower()
        tpd = int(data.get('tpowerdown'))
        if axis not in ['x','y','both']:
            return jsonify({'status': 'error', 'message': 'axis must be x, y, or both'}), 400
        ok = True
        if axis in ['x','both']:
            ok = ok and stepper_controller.tmc_set_tpowerdown('x', tpd)
        if axis in ['y','both']:
            ok = ok and stepper_controller.tmc_set_tpowerdown('y', tpd)
        if not ok:
            return jsonify({'status': 'error', 'message': 'Failed to write TPOWERDOWN'}), 400
        return jsonify({'status': 'success'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 400
